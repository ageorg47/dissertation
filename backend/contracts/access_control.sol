...
contract access_control {
    ...
    address[] public registeredManagerAddresses;
    uint[] public registeredDevicesIds;
    ...
    // Function to register a manager
      function registerManager(address managerAddress, string memory name) public {
        ...
        registeredManagerAddresses.push(managerAddress);
    ...
    function registerDevice(string memory name, uint deviceId, address deviceAddress) public {
        ...
        registeredDevicesIds.push(deviceId);
    ...
    for (uint i = 0; i < registeredDevicesIds.length; i++) {
        if (devices[registeredDevicesIds[i]].deviceAddress == constrainedDeviceAddress) {
            constrainedDeviceFound = true;
            constrainedDeviceIndex = registeredDevicesIds[i];
            break;
        }
    }
    ...
    uint newDeviceIndex = 0;
     for (uint i = 0; i < registeredDevicesIds.length; i++) {
        if (devices[registeredDevicesIds[i]].deviceAddress == newDeviceAddress) {
            newDeviceFound = true;
            newDeviceIndex = registeredDevicesIds[i];
            break;
        }
    }
    ...
    function deregisterManager (address managerAddress) public {
        ...
        // Remove manager address from the array
        for (uint i = 0; i < registeredManagerAddresses.length; i++) {
         if (registeredManagerAddresses[i] == managerAddress) {
            // Shift the remaining elements one position to the left
            for (uint j = i; j < registeredManagerAddresses.length - 1; j++) {
                registeredManagerAddresses[j] = registeredManagerAddresses[j+1];
            }
            // Remove the last element
            registeredManagerAddresses.pop();
            break;
        }
    }

        //Remove manager from the list of managed devices for each device that has the current manager
        for (uint i = 0; i < registeredDevicesIds.length; i++) {
            for (uint j = 0; j < devices[registeredDevicesIds[i]].managers.length; j++) {
                if (devices[registeredDevicesIds[i]].managers[j] == managerAddress) {
                    removeManagerFromDevice(registeredDevicesIds[i], managerAddress);
                }
            }
        }
    ...
    function removeDevice (uint deviceId) public {
        ...
        for (uint i = 0; i < registeredDevicesIds.length; i++) {
         if (registeredDevicesIds[i] == deviceId) {
            // Shift the remaining elements one position to the left
            for (uint j = i; j < registeredDevicesIds.length - 1; j++) {
                registeredDevicesIds[j] = registeredDevicesIds[j+1];
            }
            // Remove the last element
            registeredDevicesIds.pop();
            break;
        }
    }
    ...
    function revokePermission(address manager, address constrainedDevice, address newDevice) public {
        ...
        bool constrainedDeviceFound = false;
          for (uint i = 0; i < registeredDevicesIds.length; i++) {
        if (devices[registeredDevicesIds[i]].deviceAddress == constrainedDevice) {
            constrainedDeviceFound = true;
            constrainedDeviceIndex = registeredDevicesIds[i];
            break;
        }
        ...
        bool newDeviceFound = false;
         for (uint i = 0; i < registeredDevicesIds.length; i++) {
        if (devices[registeredDevicesIds[i]].deviceAddress == newDevice) {
            newDeviceFound = true;
            newDeviceIndex = registeredDevicesIds[i];
            break;
        }
        ...
        bool deviceFound = false;
          for (uint i = 0; i < registeredDevicesIds.length; i++) {
        if (devices[registeredDevicesIds[i]].deviceAddress == deviceAddress) {
            deviceFound = true;
            deviceIndex = registeredDevicesIds[i];
            break;
        }
        }
        ...
        //Function to return the Ids of the registered device
        function getDeviceIds() public view returns (uint[] memory) {
        return registeredDevicesIds;
        }

        //Function to return the addresses of the registered managers
        function getManagerAddresses() public view returns (address[] memory) {
        return registeredManagerAddresses;
        }

        //Function to return the addresses of the registered devices
        function getDeviceAddresses() public view returns (address[] memory) {
        address[] memory deviceAddresses = new address[](registeredDevicesIds.length);
        for (uint i = 0; i < registeredDevicesIds.length; i++) {
            deviceAddresses[i] = devices[registeredDevicesIds[i]].deviceAddress;
        }
