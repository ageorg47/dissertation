...
router.get("/register-manager", async (req, res) => {
  ...
  try {
    ...
    res.send({ message: `Manager registered.<br/>Transaction Hash: ${result.transactionHash}` });
  } catch (error) {
    //Get the error message from the contract require statement and send it
    //back to the client
...
router.get("/register-device", async (req, res) => {
  ...
  try {
    ...
    res.send({ message: `Device registered.<br/>Transaction Hash: ${result.transactionHash}` });
  } catch (error) {
    //Get the error message from the contract require statement and send it
    //back to the client
    res.send({ message: error.reason });
...
router.get("/add-manager", async (req, res) => {
  ...
  try {
    ...
    res.send({ message: `Manager added.<br/>Transaction Hash: ${result.transactionHash}` });
  } catch (error) {
    //Get the error message from the contract require statement and send it
    //back to the client
    res.send({ message: error.reason });
...
router.get("/remove-manager", async (req, res) => {
  ...
  try {
    ...
    res.send({ message: `Manager removed.<br/>Transaction Hash: ${result.transactionHash}` });
  } catch (error) {
    //Get the error message from the contract require statement and send it
    //back to the client
    res.send({ message: error.reason });
...
router.get("/add-rule", async (req, res) => {
  ...
  try {
    ...
    res.send({ message: `Rule added.<br/>Transaction Hash: ${result.transactionHash}` });
  } catch (error) {
    //Get the error message from the contract require statement and send it
    //back to the client
    res.send({ message: error.reason });
...
router.get("/deregister-manager", async (req, res) => {
  ...
  try {
    ...
    res.send({ message: `Manager de-registered.<br/>Transaction Hash: ${result.transactionHash}` });
  } catch (error) {
    //Get the error message from the contract require statement and send it
    //back to the client
    res.send({ message: error.reason });
...
router.get("/deregister-device", async (req, res) => {
  ...
  try {
    ...
    res.send({ message: `Device de-registered.<br/>Transaction Hash: ${result.transactionHash}` });
  } catch (error) {
    //Get the error message from the contract require statement and send it
    //back to the client
    res.send({ message: error.reason });
...
router.get("/revoke-permission", async (req, res) => {
  ...
  try {
    ...
    res.send({ message: `Permission revoked.<br/>Transaction Hash: ${result.transactionHash}` });
  } catch (error) {
    //Get the error message from the contract require statement and send it
    //back to the client
    res.send({ message: error.reason });
...
//Handle requests to get IDs of the registered devices
router.get("/get-device-ids", async (req, res) => {
  let deviceIds;
  try {
    deviceIds = await contractCalls.getDeviceIds();
    res.json({
      deviceIds: deviceIds,
    });
  } catch (error) {
...
//Handle requests to get the addresses of the registered managers
router.get("/get-manager-addresses", async (req, res) => {
  let managerAddresses;
  try {
    managerAddresses = await contractCalls.getManagerAddresses();
    res.json({
...
//Handle requests to get the addresses of the registered devices
router.get("/get-device-addresses", async (req, res) => {
  let deviceAddresses;
  try {
    deviceAddresses = await contractCalls.getDeviceAddresses();
    res.json({
