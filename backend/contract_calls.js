...
//Function to get the IDs of the registered devices
async function getDeviceIds() {
  try {
    let deviceIds;
    deviceIds = await contract.methods.getDeviceIds().call();
    return deviceIds;
  } catch (error) {
    throw error;
  }
...
//Function to get the addresses of the registered managers
async function getManagerAddresses() {
  try {
    let managerAddresses;
    managerAddresses = await contract.methods.getManagerAddresses().call();
...
//Function to get the addresses of the registered devices
async function getDeviceAddresses() {
  try {
    let deviceAddresses;
    deviceAddresses = await contract.methods.getDeviceAddresses().call();
